...
class Text2VideoDataset:
    def __init__(
        ...
        def sample_frames_from_binary(sample):
            ...
            start_frame_idx = 0
            if num_frames - self.duration * self.frame_interval <= 0:
                number = self.duration * self.frame_interval - num_frames + 1
                for j in range(number):
                    reader = np.append(reader, reader[-1][None,...],axis=0)
                num_frames = reader.shape[0]
        ...
        print(f"num_worker_batches: {num_worker_batches},\t num_batches: {num_batches},\t num_samples: {num_samples}")
