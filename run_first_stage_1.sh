...
for variable  in {0..75..5}
    do
        ...
        PREV_COUNT=$(( ($COUNT / 4) * 4 ))
        if [ $PREV_COUNT -eq 0 ]
        then
            PREV_COUNT=$[$PREV_COUNT-$PREV_DIFFENCE]
        fi
        ...
        pkill python
        pkill wandb
        export SEED=$[$variable+$END_DIFFERENCE]
        ...
        export PREV_TEACHER_UNET=./work_dirs/modelscopet2v_distillation_${PREV_COUNT}/checkpoint-final
